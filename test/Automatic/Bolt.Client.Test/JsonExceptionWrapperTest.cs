...
namespace Bolt.Client.Test
{
    public class JsonExceptionWrapperTest
    {
        ...
        public void UnwrapException_Ok()
        {
            ...
            var writeContext = new WriteExceptionContext(new ClientActionContext(), ex);
            var serializedException = Subject.Write(writeContext);
            var readContext = new ReadExceptionContext(new ClientActionContext(), serializedException);
