...
----| <Contracts>.Server.csproj		- server side code (Katana OWIN Host)
...
Link <Contract>.Contract.cs, <Contract>.Server.cs files and use folowing code in your Startup class:
...
Link <Contract>.Contract.cs, <Contract>.Client.cs files and use folowing code:
	...
	var config = new ClientConfiguration(new ProtocolBufferSerializer(), new JsonExceptionSerializer(new JsonSerializer()), new DefaultWebRequestHandlerEx()); // needs to match server configuration
...
1. Add 'Configuration.json' file to <Contracts>.csproj
   ...
   will generate configuration for all interfaces in the assembly. You can then customized the configuration by your needs.
