...
namespace Bolt.Server.Internal
{
    public static class MethodInvokerBuilder
    {
        public static Func<object, object[], object> Build(Type contract, MethodInfo method)
        {
            ...
            // execute the call
            MethodCallExpression body = Expression.Call(instanceAccessExpression, method,
                BuildParameters(parameters, parametersArrayParam));

            if (method.ReturnType == typeof(void))
            {
                // void method, compile the lamda that returns no value
                Action<object, object[]> lambda = Expression.Lambda<Action<object, object[]>>(body,
                    instanceParam, parametersArrayParam).Compile();
            ...
            // compile lambda
            return
                Expression.Lambda<Func<object, object[], object>>(Expression.Convert(body, typeof(object)),
                    instanceParam, parametersArrayParam)
                    .Compile();
        }
        ...
        public static Func<Task, object> BuildTaskResultProvider(Type resultType)
        {
            ...
            return Expression.Lambda<Func<Task, object>>(Expression.Convert(body, typeof (object)),
                taskParam).Compile();
        }
        ...
        private static Expression[] BuildParameters(ParameterInfo[] parameters, ParameterExpression parametersArrayParam)
        {
            ...
            {
                result[i] = Expression.Convert(Expression.ArrayAccess(parametersArrayParam, Expression.Constant(i)),
